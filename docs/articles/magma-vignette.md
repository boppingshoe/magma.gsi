...
distributions[^2] of the population and age classes. The full
...
chain contains a tibble with age classes ($`\times`$ iterations
$`\times`$ \# of districts $`\times`$ \# of subdistricts $`\times`$ \#
of weeks) as rows and populations as columns. I call the output in “raw”
format because it has not been summarized. The output also contains the
...
(Gelman-Rubin statistic, or $`\hat R`$) passes the test but $`N_{eff}`$
...
individual $`x`$ elements. And $`x_{m,l,j}`$ is the count of allele
$`j`$ in locus $`l`$ for individual fish $`m`$, where
$`m \in \{1,2,...,M\}`$, $`l \in \{1,2,...,L\}`$, and
$`j \in \{1,2,...,J_l\}`$ depends on locus $`l`$.
...
$`y_{k,l,j}`$ is the count of allele $`j`$ in locus $`l`$ for a sample
of size $`n_{k,l}`$ collected from baseline population $`k`$, where
$`k \in \{1,2,...,K\}`$.

For both mixture and baseline, it is assumed that allele counts in each
locus follow a multinomial distribution[^3]. Using a made-up example, in
a baseline, there are two alleles in locus 1 for population 2. Counts
for the two alleles are $`y_{1,2,1},  y_{1,2,2}`$, and they follow a
multinomial distribution with parameters $`q_{1,2,1}, q_{1,2,2}`$ and
...
parameters; therefore, we place a Dirichlet[^4] prior distribution on
$`q_{1,2,1}, q_{1,2,2}`$ with hyperparameters[^5]
$`\beta_{1,1}, \beta_{1,2}`$, where $`\beta_{1,1} = \beta_{1,2} = 1/2`$
based on the number of alleles for locus 1.
...
where $`\mathbf \beta = 1/J_l`$ is a commonly used prior for
$`\mathbf q`$.
...
is unknown so it needs to be estimated. Here we let $`\mathbf z_m`$
represent the population identify for the $`m`$^(th) mixture individual.
$`\mathbf z_m`$ is composed of 0’s and an 1 with a length $`K`$
(e.g. number of baseline populations). $`z_{m,k} = 1`$ if individual
$`m`$ belongs to population $`k`$, and $`z_{m,k} = 0`$ otherwise. In a
made-up example, $`\mathbf z_{100} = \{0, 0, 1, 0, 0\}`$ means that
there are only five populations, and individual fish \#100 comes from
population 3.

We place a multinomial distribution on
$`z_{m,1}, z_{m,2}, ..., z_{m,K}`$ with size 1 and probabilities equal
to population proportions $`p_1, p_2, ..., p_K`$. We specify a Dirichlet
prior distribution on $`p_1, p_2, ..., p_K`$ with hyperparameters
$`\alpha_1, \alpha_2, ..., \alpha_K`$, where
$`\alpha_1 = \alpha_2 = ... = \alpha_K = 1/K`$. We express $`\mathbf z`$
as follows:
...
where $`\mathbf{ploidy} = ploidy_1, ploidy_2, ..., ploidy_L`$ denotes
ploidy for each locus. We use $`\circ`$ to denote the element-wise
product.
...
distribution of mixture sample, $`\mathbf x_m`$. That is, baseline
allele frequencies are not being updated in the model. The result of
that, $`\mathbf x_m`$ takes a form of a compound Dirichlet-multinomial
...
to attempt proving the theory behind the conditional GSI model in this
...
genotyped $`M^{(2)}`$; and 3) the number of otolith-marked fish
$`M^{(3)}`$. Note that $`M = M^{(1)} + M^{(2)} + M^{(3)}`$.
...
sample-component. Compartmentalized $`\mathbf z^{(i)}_m`$, where
...
addition of hatchery populations, $`\mathbf z^{(i)}_m`$ and parameters
$`\mathbf p`$ are now of length $`K+H`$. $`p_1, p_2, ..., p_{K+H}`$ have
a Dirichlet distribution with hyperparameters
$`\alpha_1, \alpha_2, ..., \alpha_{K+H} = 1/(K+H)`$. We express
...
that the genetic baseline $`y`$ is unchanged from the Pella-Masuda
model; however, no age-class baseline is available for any population.
As described earlier, some fish in the mixture sample are aged and some
are not. However, in this document we will pretend that all fish are
aged so that the notation would be less headache-inducing. The
fundamental concept would still be the same when not all fish were aged,
only with more complicated notations.

Age class is identified as $`c`$, where $`c \in \{1,2,...,C\}`$.
$`\mathbf a`$ denotes the age identities of mixture fish. Let
$`\mathbf a_m`$ represent the age identify for the $`m`$^(th) mixture
individual. $`\mathbf a_m`$ are also partitioned into three subsets,
$`\mathbf a^{(1)}`$, $`\mathbf a^{(2)}`$, and $`\mathbf a^{(3)}`$,
according to the sample-components. However, it is not necessary to
compartmentalize $`\mathbf a_m`$ for the most part in the model. Age
identity and population identity have a similar structure, and
$`\mathbf a_m`$ is also composed of 0’s and an 1 but with a length
$`C`$. $`a_{m,c}`$ is the age identity for the $`m`$^(th) mixture
individual in the $`c`$^(th) age class, where $`a_{m,c} = 1`$ if
individual $`m`$ has age class $`c`$, and $`a_{m,c} = 0`$ otherwise. For
example, if there were three age classes and fish \#6 was age 3, then
$`\mathbf a_6 = \{0, 0, 1\}`$.

We place a multinomial distribution on
$`a_{m,1}, a_{m,2}, ..., a_{m,C}`$ with size 1 and probabilities equal
to age-class frequencies $`\mathbf z_m \cdot \mathbf \pi`$, where
$`\mathbf \pi`$ denotes age-class frequencies within each population 1
through $`K+H`$. You can picture $`\mathbf \pi`$ as a matrix with
$`K+H`$ populations of rows and $`C`$ age classes of columns. Therefore,
when multiplying population identities,
$`z_{m,1}, z_{m,2}, ..., z_{m,K}`$, and age-class frequencies,
$`\mathbf \pi_{1,\{1,2,...,C\}}, \mathbf \pi_{2,\{1,2,...,C\}}, ..., \mathbf \pi_{K+H,\{1,2,...,C\}}`$,
only age-class frequencies within the population which individual $`m`$
belong to would remain while the rest goes to zero.

We specify a Dirichlet prior on $`\pi_{k,1}, \pi_{k,2}, ..., \pi_{k,C}`$
with hyperparameters $`\gamma_1, \gamma_2, ..., \gamma_C = 1/C`$. We
express $`\mathbf a`$ and prior for $`\mathbf \pi`$ as follows:
...
distributions for
$`\mathbf p, \mathbf q, \mathbf z^{(1)}, \mathbf z^{(2)}, \mathbf\pi|\mathbf x, \mathbf y, \mathbf z^{(3)}, \mathbf a, \mathbf\alpha, \mathbf\beta, \mathbf\gamma`$.
...
distributions[^6], each parameter in $`\mathbf \theta`$ can be sampled
...
distributions for $`\mathbf q`$, $`\mathbf p`$, and $`\mathbf\pi`$:
...
Likelihood $`p(\mathbf x|\mathbf z^{(1)}, \mathbf q)`$ can be derived in
two steps. The first step we conditioned the likelihood on
$`\mathbf z^{(1)}`$ so that
...
probability density has a kernel[^7] of
...
Lastly, $`p(\mathbf \pi|\mathbf a,\mathbf z,\mathbf\gamma)`$ is also
Dirichlet-distributed due to a Dirichlet prior
$`p(\mathbf \pi|\mathbf\gamma)`$ and a multinomial likelihood
$`p(\mathbf a|\mathbf z,\mathbf\pi)`$.
...
where likelihood
$`h(\mathbf a_m|\mathbf\pi_k) \propto \displaystyle \prod^C_{c=1}\pi^{a_{m,k}}_{k,c}`$
is the product of relative frequency of age class for individual $`m`$
in population $`k`$. Plugging in $`h(\mathbf a_m|\mathbf\pi_k)`$,
...
$`\mathbf z^{(1,2)}_m|\mathbf p, \mathbf q, \mathbf x^{(1,2)}_m, \mathbf\pi, \mathbf a^{(1,2)}_m`$,
the population identity for individual fish $`m`$ (in components 1 and
2) given the population proportions, genotype, and age. If the
probability of fish $`m`$ belong to population $`k`$ is $`p_k`$, and the
likelihood of observing relative frequency of genotype and age class for
fish $`m`$ in population $`k`$ is
$`f(\mathbf x^{(1,2)}_m|\mathbf q_k) \cdot h(\mathbf a^{(1,2)}_m|\mathbf\pi_k)`$,
then the probability of fish $`m`$ belong to population $`k`$ given the
population proportions genotype, and age is
$`\displaystyle \frac{p_k \cdot f(\mathbf x^{(1,2)}_m|\mathbf q_k) \cdot h(\mathbf a^{(1,2)}_m|\mathbf\pi_k)}{\sum^K_{k'=1}p_{k'} \cdot f(\mathbf x^{(1,2)}_m|\mathbf q_{k'}) \cdot h(\mathbf a^{(1,2)}_m|\mathbf\pi_{k'})}`$.
...
where
$`p'_{m,k} = p_k \cdot f(\mathbf x^{(1,2)}_m|\mathbf q_k) \cdot h(\mathbf a^{(1,2)}_m|\mathbf\pi_k)`$.
We draw the initial values for $`\mathbf q_k`$ and $`\mathbf \pi_k`$
...
$`\mathbf q^{(0)}`$, and $`\mathbf \pi^{(0)}`$. If not all fish were
aged, $`\mathbf a^{(1,2)(0)}_m`$ at the initial step would contain all
...
1.  Determine the population membership of mixture individuals,
    $`{{\mathbf z}^{(1,2)}_m}^{(t)}|\mathbf p^{(t-1)}, \mathbf q^{(t-1)}, \mathbf x^{(1,2)}_m, \mathbf\pi^{(t-1)}, {{\mathbf a}^{(1,2)}_m}^{(t-1)} \sim Mult(1, \mathbf {p'}_m)`$.

2.  If not all individuals were aged, determine memberships of age
    classes for those with unknown age,
    $`\mathbf a^{(t)}_m|\mathbf z^{(t)}_m, \mathbf \pi^{(t-1)} \sim Mult(1, \mathbf z^{(t)}_m \mathbf\pi^{(t-1)})`$.

3.  Draw updated values for $`\mathbf q^{(t)}`$, $`\mathbf p^{(t)}`$,
    and $`\mathbf\pi^{(t)}`$ from
    $`p(\mathbf q|\mathbf x,\mathbf y,{{\mathbf z}^{(1)}}^{(t)},\mathbf\beta)`$,
    $`p(\mathbf p|\mathbf z^{(t)},\mathbf \alpha)`$, and
    $`p(\mathbf \pi|\mathbf a^{(t)},\mathbf z^{(t)},\mathbf \gamma)`$
    respectively.

$`T`$ should be large enough to ensure the sampler chain converges to
the posterior distribution of the parameters. Usually it takes thousands
of iterations. That completes the Gibbs sampler process. Whew.

Implementing the conditional GSI model only requires a slight
modification from the above algorithm. Basically,
$`f(\mathbf x^{(1,2)}_m|\mathbf q_k)`$ would only need to be derived
once in the beginning of the process, and $`\mathbf q`$ would no longer
need to be updated in step 3. Everything else would stay the same.
