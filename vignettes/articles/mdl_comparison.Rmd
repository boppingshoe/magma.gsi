...
```{r, include = FALSE}
pacman::p_load(DT, tidyverse, wesanderson)

magma_sims00 <- readRDS("D:/bobby_adfg/projects/magma/cond_gsi_vs_fully_bayes/magma_sims00.Rds")
...
```{r mse, fig.width=9, fig.height=6, echo=FALSE}
err_table %>%
...
```{r bias, fig.width=9, fig.height=6, echo=FALSE}
err_table %>%
...
```{r deviations, fig.width=9, fig.height=6, echo=FALSE}
err_table %>%
...
```{r gr, fig.width=9, fig.height=6, echo=FALSE}
magma_sims_gr <-
  magma_sims00 %>%
  bind_rows() %>%
  select(-mean) %>%
...
```{r, include = FALSE}
lynn_sims00 <- readRDS("D:/bobby_adfg/projects/magma/cond_gsi_vs_fully_bayes/lynn_sims/lynn_sims00_2.Rds") %>%
  bind_rows() %>%
  mutate(itr = itr + 50) %>%
  bind_rows(readRDS("D:/bobby_adfg/projects/magma/cond_gsi_vs_fully_bayes/lynn_sims/lynn_sims00.Rds"))
...
```{r gr lynn, fig.width=9, fig.height=6, echo=FALSE}
lynn_sims_gr <-
  lynn_sims00 %>%
  select(-mean) %>%
  filter(!type %in% c("true_prop", "mix_prop"), !is.na(GR)) %>%
