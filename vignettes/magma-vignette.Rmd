...
magma_out <-
  magmatize_mdl(dat_in = yomamafat,
                ...
                keep_burn = TRUE, flat_age_priors = FALSE,
...
MAGMA estimates age/stock composition at the lowest stratum level (i.e., stat weeks), but sums up the lower strata (weighted by harvest numbers) to provide summaries at the district or subdistrict level. Setting `summ_level = "district"` provides the summaries at the district level, and `"subdistrict"` at the subdistrict level.

The summarized output is organized as list items. The items with `_summ` are the summary table with posterior means, median, CI's and convergence diagnostics (Gelman-Rubin and effective size). For the district level, `age_summ` item provides summaries of age composition for each stock within a district. `pop_summ_all` and `pop_summ` provide stock proportion estimates for a district and summing across statistical weeks within a district, respectively. For the subdistrict level, `age_summ` item provides summaries of age composition for each stock within each subdistrict. `pop_summ_all` and `pop_summ` provide stock proportion estimates for each subdistrict and for each statistical week within a subdistrict, respectively. 
