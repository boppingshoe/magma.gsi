...
yomamafat <- magmatize_data(wd = getwd(), age_classes <- c(13, 21, 22, 23, 31, 32, "other"), save_data = FALSE)
...
Burn-ins are excluded in the summary calculations even if a user choose to keep the burn-in output. There is an option to make the age priors slightly more influential in the MCMC sampling. Lastly, `keep_burn`, `flat_age_prior`, and `out_path` use the default values if not specified by the user.

`flat_age_prior = TRUE` set an equal weight across the age groups. Setting `flat_age_prior` to `FALSE` will concentrate the prior weight on the major age groups and force the unobserved age groups to (near) zero.
