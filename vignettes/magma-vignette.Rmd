...
where $\mx \beta = 1/J_l$ is a commonly used prior for $\mx q$.
...
$\mx x_m \sim CDM(\mx{ploidy}, \mx z_m \circ \mx v)$,
...
We specify a Dirichlet prior on $\pi_{k,1}, \pi_{k,2}, ..., \pi_{k,C}$ with hyperparameters $\gamma_1, \gamma_2, ..., \gamma_C = 1/C$. We express $\mx a$ and prior for $\mx \pi$ as follows:
...
Estimation of MAGMA parameters requires deriving the conditional distributions for $\mx p, \mx q,\mx z^{(1)}, \mx z^{(2)}, \mx\pi|\mx x, \mx y, \mx z^{(3)}, \mx a, \mx\alpha, \mx\beta, \mx\gamma$. In the next section, we will introduce the concepts and an algorithm to sample the posterior distribution.
