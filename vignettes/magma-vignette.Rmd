...
*mixture.RData* contains the `.gcl` file(s) for the mixture (samples with genetic information) and the fishery name (as a character string). Including the fishery name here is optional and can be specified in the `magmatized_data()` function instead. If you specified at both places with different fishery names, the one in the `mixture.RData` will take precedence. This option allows users to summarize MAGMA output under different setups without rerunning the model again.
...
`cond_gsi` sets the option to run MAGMA model in the conditional GSI or fully Bayesian algorithm. `nadapt` allows for a "warm up" run in the conditional GSI mode before running the model in the fully Bayesian mode.

`flat_age_prior = TRUE` set an equal weight across the age groups. Setting `flat_age_prior = FALSE` will concentrate the prior weight on the major age groups and force the undetected age groups to (near) zero.
...
For the stock proportions at the district level, estimates of each statistical week are summed up across subdistricts within each district. There is also a summary that summed up with all estimated proportions of subdistrict and statistical weeks within each district.

<!-- figure explain summary schemes -->
...
3. Stochastically assign each wild fish with genotypes to a wild population of origin based likelihood derived from its genotypic frequency and population proportion. If its age is unobserved, assign an age based on the age composition of its assigned population;
...
4. Stochastically assign each wild fish without genotypes to a population of origin based on population proportion. If its age is unobserved, assign an age based on the age composition of its assigned population; and
