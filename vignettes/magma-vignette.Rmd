...
*group_namesXXX.txt* and *groupsXXX.txt* are saved with fishery extension. For example, *group_namesFAKE.txt* and *groupsFAKE.txt* are files for "FAKE" fishery. *group_namesXXX.txt* contains the names and identifying numbers (i.e. groupvec) that are associated with the reporting groups. *groupsXXX.txt* contains the group identifying number for each population in the data. The following shows the format of *group_namesXXX.txt* and *groupsXXX.txt*.
...
*harvestXXX.txt* is also saved with a fishery extension. For "FAKE", harvest is saved as *harvestFAKE.txt*. Harvest file contains the number of harvest for each sampling week in each district and sub-district. The following shows an example for the "FAKE" fishery harvest. Note that the column names in the harvest file have to be in all capital letters.
...
*metadata.txt* contains the information where (district and sub-district) and when (week) each fish in the dataset was collected. Each fish is identified by `SILLY_VIAL`, an unique identifier used by GCL (I think?). *metadata.txt* also contains information for age and origin for each fish, if they are known. The following shows the format for *metadata.txt*. Note that the column names have to be in all capital letters.
...
*mixture.RData* contains the information for the mixture population collected with fishery harvest. In the *.RData* file, there are the fishery name (as a character string) and `.gcl` file(s) for the mixture.

`magmatize_data()` has the option to save or not to save the input data. The default is `save_data = TRUE`, and it will save the data as *magma_dataXXX.RData* file in the *data* folder in the working directory. This file is saved with the fishery extension (e.g. *magma_dataFAKE.RData*). *magma_dataXXX.RData* is saved for record keeping, and it is not required for running MAGMA. To run MAGMA, the input file need to be organized as a list object. And *magma_dataXXX.RData* has all the information needed to make that input data list object.

User can identify the major age classes for the analysis, or let MAGMA choose what age classes to estimate. If no age class is identified, MAGMA goes in the metadata and find the youngest freshwater age and the oldest saltwater age and expand the age classes. User and specify a "0X" age to catch all 0 freshwater ages. And an "other" age class as a catch-all for the unidentified classes.
...
yomamafat <- magmatize_data(wd = dirt, age_classes <- c(13, 21, 22, 23, 31, 32), save_data = FALSE)
...
Burn-ins are excluded in the calculations for summaries even if a user choose to keep the burn-in output. There is an option to make the age priors slightly more influential in the MCMC sampling. Lastly, `keep_burn`, `age_prior`, and `out_path` use the default values if not specified by the user.
...
NEW! For big fisheries like TBR, output can be too large for our work laptop to process. The summary process has been broken down into three smaller step to manage memory use. The three steps are done using `magmatize_summ_bd1()`, `magmatize_summ_bd2()`, and `magmatize_summ_bd3()`.
...
The summarized output is organized as list items. The items with "`_prop`" are the posterior samples/simulations (i.e., trace) for age/population proportions in a data frame. The data frame contains output from all MCMC chains stacking together (using `bind_rows()`). In this example, I run two chains with 50 iterations, burn-ins of 25, and no thinning. Because I chose to keep the burn-ins, the results are 50 rows of output in each chain. Stacking the two chains we end up with 100 rows of posterior samples.
...
$N_{eff}$ (or `n_eff` in the summary table) represents an estimate of independent sample size in the posterior sample. A large $N_{eff}$ is considered better than a small $N_{eff}$. Some says you would need at least blah blah number to properly estimate the credible intervals, but there is no "official" number to go by. My experience tells me to look at $N_{eff}$ together with other diagnostics. Sometimes you may see GR (Gelman-Rubin statistic, or $\hat R$ in statistics literature) passes the test but $N_{eff}$ is small. You may want to investigate the trace plot for that particular output. Also, when the simulation iterations are small, as in this example, the results for $N_{eff}$ can get wacky.


### trace plot

The posterior samples outputs can be used to make trace plots with your own code or with function `tr_plot()`. If using `tr_plot()`, you need to specify the amount of burn-ins and thinning you had when you ran the model. The defaults are zero burn-ins and thinning of one. `tr_plot()` outputs one data frame (e.g. sampling period) at a time. The burn-in portion of the output is shaded in red.
